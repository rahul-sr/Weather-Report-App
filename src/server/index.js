...
const getWeatherData = (
  ...
  let wd = weatherData.find(
    (ds) => ds.id === Number(cityId)
  );
  let convertor =
    tempUnits === "C" ? k2c : k2f;
  ...
  return wd
    ? {
        ...
        feels_like: convertor(
          wd.main.feels_like
        ),
        pressure: wd.main.pressure,
        humidity: wd.main.humidity,
        conditions:
          wd.weather[0].description.toUpperCase(),
...
export function weatherServer({
  ...
  return new Server({
    ...
    routes() {
      this.urlPrefix =
        "https://api.weatherserver.com";
      this.namespace = "weather";

      this.get(
        "/current/:cityId/:tempUnits",
        (_, request) => {
          const city =
            Number(
